...
class EntryController {
    static getAllLive = async (req: Request, res: Response, next: NextFunction) => {
        try {

            let entries;

            await axios.get('http://api.publicapis.org/entries')
                .then(response => {
                    console.log('res', response.data)
                    entries = response.data;
                });

            return sendResponse(res, entries, locale('ENTRIES_GET_ALL_LIVE_SUCCESS'), RESPONSE_SUCCESS, RESPONSE_CODE.SUCCESS);
        } catch (error) {
